...
class Quest:
    active: bool
    given_by: int        # pid of the questgiver object
    target_pid: int      # pid of the target NPC
    target_name: str
    text: str
...
class LocalStableAudioProvider:
    ...
    def generate(self, spec_like: Any, prepend: Optional[List[str]] = None) -> GeneratedLoop:
        ...
        return GeneratedLoop(out_path, seconds_total, bpm, getattr(spec_like, "key_mode", "Unknown"), prompt,
...
class RegionMap:
    ...
    def _place_pois(self):
        ...
        # --- Place special questgiver beacon in a neighbor zone of the start ---
        ...
        for qz in neighs:
            ...
            if not tiles: 
                continue
            tile = tiles[0]
            beacon = POI(next_id, "object", "Beacon of Names", "questgiver", tile, qz, rarity=99, kind_key="beacon")
...
class AudioService:
    ...
    def request_zone(self, zid: int, priority: Optional[Tuple[int,int]]=None, force: bool=False):
        ...
        if not force and (zr.generating or zr.loop): return
        prio = priority or self._priority_for_zone(zid)
        with self._lock:
            token = next(self._counter)
            self._pending[("zone",zid)] = token
            heapq.heappush(self._heap, (prio, token, {"kind":"zone","id":zid,"token":token}))

    def request_poi(self, pid: int, priority: Optional[Tuple[int,int]]=None, force: bool=False):
        poi = self.m.map.pois[pid]
        if not force and (poi.generating or poi.loop): return
        prio = priority or self._priority_for_poi(pid)
        with self._lock:
            token = next(self._counter)
            self._pending[("poi",pid)] = token
            heapq.heappush(self._heap, (prio, token, {"kind":"poi","id":pid,"token":token}))
    ...
    # ---------- events ----------
    def on_zone_changed(self, old_z: int, new_z: int):
        # audible may change via POI, but ensure zone source available
    ...
    def _generate_for(self, src: Tuple[str,int]) -> Optional["GeneratedLoop"]:
        ...
        if k == "zone":
            ...
            # derive a POI-focused spec (funk bias, optional hints)
            ...
            return self.provider.generate(zspec)
...
class GameView:
    ...
    def draw(self, screen: pygame.Surface, record_armed: bool, recorder_active: bool,
        ...
        # Draw tiles
        # Build visible tiles grouped by zone id ------------- NEW
        ...
        for zy in range(min_zy, max_zy+1):
            for zx in range(min_zx, max_zx+1):
                ...
                # tile fill (cached per-zone color)
                base_col = self.m.map.zone_color.get(zid, (60,60,80))
                ...
                if pid:
                    poi = self.m.map.pois[pid]
                    color = (255,220,90) if poi.name=="Boss Skuggs" else \
                            ((200,255,200) if poi.kind=="npc" else (200,200,255))
                    pygame.draw.circle(screen, color, (rect.centerx, rect.centery), 10)
                    self._blit_label(screen, poi.name, rect.centerx + 10, rect.centery - 10)
        
                # audible source marker
                if active_tile == (zx,zy):
                    pygame.draw.rect(screen, (200,120,255), rect, width=3, border_radius=12)
        
                # collect for perimeter/name pass
                visible_by_zone.setdefault(zid, []).append((zx, zy))
        
        # One overlay for all perimeters (fast)
        ...
        # One zone label per zone (anchor = top-left visible tile)
        for zid, tiles in visible_by_zone.items():
            anchor = min(tiles)
            ax = int(anchor[0]*TILE_W - cam_x) + 12
            ay = int(anchor[1]*TILE_H - cam_y) + 10
            self._blit_label(screen, self.m.map.zones[zid].spec.name, ax, ay)

        
        overlay = pygame.Surface((SCREEN_W, SCREEN_H), pygame.SRCALPHA)
        # After tiles: draw a single perimeter + zone name per zone ------------- NEW
        # Replace the old per-zone call:
        for zid, tiles in visible_by_zone.items():
            zr = self.m.map.zones[zid]
            if zr.loop and not zr.error:
                self._stroke_zone_edges(overlay, tiles, cam_x, cam_y, pulse_t)
        # After loop, blit once:
        screen.blit(overlay, (0, 0))

        # zone name once at top-left visible tile (anchor)
        anchor = min(tiles)  # lexicographic: top-left
        ax = int(anchor[0]*TILE_W - cam_x) + 12
        ay = int(anchor[1]*TILE_H - cam_y) + 10
        self._blit_label(screen, zr.spec.name, ax, ay)
        
        pid = self.m.map.pois_at.get((zx,zy))
        if pid:
            poi = self.m.map.pois[pid]
            if poi.role == "questgiver":
                # big pulsing beacon
                r = 14 + int(3 * (1 + math.sin(pulse_t * 4)))
                pygame.draw.circle(screen, (255, 255, 160), (rect.centerx, rect.centery), r)
                pygame.draw.circle(screen, (255, 240, 80), (rect.centerx, rect.centery), r+6, width=3)
                pygame.draw.circle(screen, (255, 200, 40), (rect.centerx, rect.centery), r+10, width=2)
                self._blit_label(screen, poi.name, rect.centerx + 10, rect.centery - 10)
            else:
                color = (255,220,90) if poi.name=="Boss Skuggs" else ((200,255,200) if poi.kind=="npc" else (200,200,255))
                pygame.draw.circle(screen, color, (rect.centerx, rect.centery), 10)
                self._blit_label(screen, poi.name, rect.centerx + 10, rect.centery - 10)
        # POI names (small labels) -------------------------------- NEW
        # for pid, poi in self.m.map.pois.items():
        #     tx, ty = poi.tile
        #     # skip if offscreen
        #     if not (min_zx <= tx <= max_zx and min_zy <= ty <= max_zy):
        #         continue
        #     cx = int(tx*TILE_W - cam_x + TILE_W//2)
        #     cy = int(ty*TILE_H - cam_y + TILE_H//2)
        #     # offset label a bit from the marker
        #     self._blit_label(screen, poi.name, cx + 10, cy - 10)


        # player
        px = int(self.m.player.px - cam_x); py = int(self.m.player.py - cam_y)
        ...
        # Quest status line
        if getattr(self.m, "quest", None) and self.m.quest.active and not self.m.quest.completed:
            hud.append(f"QUEST: Find {self.m.quest.target_name}")
        elif getattr(self.m, "quest", None) and self.m.quest.completed:
            hud.append(f"QUEST COMPLETE: Found {self.m.quest.target_name}")
...
class GameController:
    ...
    def _try_interact(self):
        # Interact with beacon if adjacent
        adj = self._adjacent_pois()
        beacon = next((p for p in adj if p.role == "questgiver"), None)
        if beacon:
            # if we already have a quest, just re-show it
            if self.model.quest and self.model.quest.active and not self.model.quest.completed:
                self.prompt_text = f"[QUEST] Seek {self.model.quest.target_name}. Stand next to them."
                self.show_prompt = True
                return
    
            # pick a target NPC from the world (avoid Boss Skuggs and the beacon itself)
            npcs = [p for p in self.model.map.pois.values() if p.kind == "npc" and p.name != "Boss Skuggs"]
            if not npcs:
                self.prompt_text = "[QUEST] The Beacon crackles, but no aliens answer…"
                self.show_prompt = True
                return
    
            target = random.choice(npcs)
            text = f"The Beacon hums:\nFind {target.name}. You'll know them by their groove."
            self.model.quest = Quest(
                active=True, given_by=beacon.pid,
                target_pid=target.pid, target_name=target.name,
                text=text, completed=False
            )
            self.prompt_text = "[QUEST] " + text
            self.show_prompt = True
    ...
    def run(self):
        ...
        while running:
            ...
            if dx or dy:
                ...
                q = getattr(self.model, "quest", None)
                if q and q.active and not q.completed:
                    # is the target adjacent?
                    px, py = self.model.player.tile_x, self.model.player.tile_y
                    t = self.model.map.pois.get(q.target_pid)
                    if t and max(abs(px - t.tile[0]), abs(py - t.tile[1])) <= 1:
                        q.completed = True
                        self.prompt_text = f"[QUEST COMPLETE] You found {q.target_name}!"
                        self.show_prompt = True
