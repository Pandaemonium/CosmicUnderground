...
class RegionMap:
    ...
    def __init__(self, w: int, h: int, seed: int):
        ...
        self.pois: Dict[int, POI] = {}        # id -> POI
        self.pois_at: Dict[Tuple[int,int], int] = {}  # tile -> poi_id
    ...
    def _place_pois(self):
        next_id = 1
        # Helper to pick interior-ish tiles (avoid border for readability)
        ...
        for zid, zr in self.zones.items():
            ...
            for _ in range(npc_n):
                ...
                name = rng.choice(["Glorp DJ","Shard Horn","Fizz Bop","Clav Duke","Holo Buzzer","Gongsmith"])
...
class AudioService:
    ...
    def __init__(self, model: WorldModel):
        ...
        def _on_player_moved(old_tile, new_tile):
            # recompute active source
            self.active_source = self._current_active_source()
            rt = self._get_runtime(self.active_source)
            # play immediately if ready; otherwise enqueue with top priority
            if rt.loop:
                self.player.play_loop(rt.loop.wav_path, rt.loop.duration_sec, fade_ms=180)
            else:
                self.request_generate(self.active_source, priority=(0,0), force=True)
            self._reprioritize_all()
        
        self.m.add_tile_changed_listener(_on_player_moved)
    ...
    def _reprioritize_all(self):
        with self._lock:
            ...
            self._heap.clear()
            self._pending.clear()
        # ensure active source goes first
        rt = self._get_runtime(self.active_source)
        ...
        px, py = self.m.player.tile_x, self.m.player.tile_y
        cz = self.m.map.zone_of[px][py]
        for zid, zr in self.m.map.zones.items():
            if zid == cz: continue
            # if any tile of zid touches cz orthogonally, it's adjacent
            adjacent = False
            for (tx, ty) in zr.tiles:
                for (nx, ny) in ((tx+1,ty),(tx-1,ty),(tx,ty+1),(tx,ty-1)):
                    if 0 <= nx < self.m.map.w and 0 <= ny < self.m.map.h:
                        if self.m.map.zone_of[nx][ny] == cz:
                            adjacent = True; break
                if adjacent: break
            if adjacent:
        ...
        # backlog (anything already pending)
        for src in items:
            rt = self._get_runtime(src)
            if rt.loop is None and not rt.generating:
...
class GameView:
    ...
    def draw(self, screen: pygame.Surface, record_armed: bool, recorder_active: bool,
        ...
        for zy in range(min_zy, max_zy+1):
            for zx in range(min_zx, max_zx+1):
                ...
                # basic tile fill
                rng = random.Random(zid*1337)
                base_col = (40 + rng.randint(0,40), 42 + rng.randint(0,30), 70 + rng.randint(0,40))
                if (zx,zy) == (self.m.player.tile_x, self.m.player.tile_y):
                    base_col = (70, 88, 140)
                pygame.draw.rect(screen, base_col, rect, border_radius=12)
        
                # ❌ remove per-tile glow (we’ll draw a single perimeter per zone)
                # if zr.loop and not zr.error:
                #     self._draw_glow(screen, rect, pulse_t)
        
                # POI marker (we'll add names later)
                ...
                # collect for zone-level perimeter/name pass ------------- NEW
        ...
        for zid, tiles in visible_by_zone.items():
            zr = self.m.map.zones[zid]
            # perimeter glow only if this zone has a ready loop and no error
            if zr.loop and not zr.error:
                self._draw_zone_perimeter(screen, tiles, cam_x, cam_y, pulse_t)
        ...
        # HUD
        hud = ["Move: WASD/Arrows | G regen zone | M cycle mood | E edit mood | N panel | P prompt | R record | Esc quit"]
    ...
    def _draw_zone_perimeter(self, screen: pygame.Surface, tiles: list[tuple[int,int]],
                             cam_x: float, cam_y: float, t: float):
        """
        Draw a glowing purple outline around the *outside* of a zone’s tile set.
        Only edges that border a different zone (or map edge) are drawn.
        """
        S = set(tiles)
        base = (170 + int(60*(0.5 + 0.5*math.sin(t*2.0))), 60, 220)  # pulse
        # Layered strokes for glow
        layers = [(8, 70), (5, 110), (2, 180)]  # (width, alpha)
        for width, alpha in layers:
            ...
            for (tx, ty) in tiles:
                ...
                rw = TILE_W - 4
                rh = TILE_H - 4
                # Draw a line ONLY if neighbor is not in the same zone
                if (tx, ty-1) not in S:  # top
                    pygame.draw.line(surf, (*base, alpha), (rx, ry), (rx+rw, ry), width)
                if (tx+1, ty) not in S:  # right
                    pygame.draw.line(surf, (*base, alpha), (rx+rw, ry), (rx+rw, ry+rh), width)
                if (tx, ty+1) not in S:  # bottom
                    pygame.draw.line(surf, (*base, alpha), (rx, ry+rh), (rx+rw, ry+rh), width)
                if (tx-1, ty) not in S:  # left
                    pygame.draw.line(surf, (*base, alpha), (rx, ry), (rx, ry+rh), width)
