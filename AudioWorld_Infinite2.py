...
class LocalStableAudioProvider(AudioProvider):
    ...
    def _prompt(self, z: ZoneSpec) -> str:
        # Rich, compact prompt: biome + species + descriptors + instruments + production tags
        desc = ", ".join(z.descriptors[:4]) if z.descriptors else z.scene
        inst = ", ".join(z.instruments[:4]) if z.instruments else ""
        tags = ", ".join(z.tags[:3]) if z.tags else ""
        parts = [
            f"{z.mood} vibes in the {z.biome}, home of the {z.species}.",
            desc,
            (f"Instruments: {inst}." if inst else ""),
            (f"Style tags: {tags}." if tags else ""),
            f"Loopable {z.bars} bars, {z.bpm} BPM, {z.key_mode}. Clean downbeats; minimal silence at edges."
        ]
