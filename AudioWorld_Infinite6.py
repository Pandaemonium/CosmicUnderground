...
AVATAR_IMAGE_PATH = r"C:\Games\CosmicUnderground\sprites\character1.png"
...
class LocalStableAudioProvider(AudioProvider):
    ...
    def generate(self, zone: ZoneSpec) -> GeneratedLoop:
        ...
        prompt, meta = promptgen.build(
            zone,
            bars=zone.bars,
            rng=random.Random(),
            intensity=0.55,
            **ctx  # keys: time_of_day, weather, heat, debt_pressure, festival, cosmic_event, neighbor_contrast_prev
        )
        ...
        return GeneratedLoop(
            out_path, seconds_total, zone.bpm, zone.key_mode, prompt,
...
class WorldModel:
    def __init__(self):
        ...
        self.time_of_day = "night"      # or "day"/"dusk"/"dawn"
        self.weather     = None         # "light rain"/"storm"/"snow"/None
        self.heat        = 0.15         # 0..1
        self.debt_pressure = 0.4        # 0..1
        self.festival      = False
    ...
    def __init__(self, model: WorldModel):
        ...
        try:
            if os.path.isfile(AVATAR_IMAGE_PATH):
                raw = pygame.image.load(AVATAR_IMAGE_PATH).convert_alpha()
                # scale to ~30% of a tile’s height (tweak to taste)
                target_h = max(32, min(int(TILE_H * 0.3), 128))
                target_w = int(raw.get_width() * (target_h / raw.get_height()))
                self.avatar = pygame.transform.smoothscale(raw, (target_w, target_h))
            else:
                print(f"[WARN] Avatar image not found: {AVATAR_IMAGE_PATH} — using fallback circle.")
        except Exception as e:
            print(f"[WARN] Failed to load avatar: {e} — using fallback circle.")
            self.avatar = None
            
    def _draw_player(self, screen: pygame.Surface, cam_x: float, cam_y: float):
        px = int(self.m.player.x - cam_x)
        py = int(self.m.player.y - cam_y)

        if self.avatar:
            rect = self.avatar.get_rect(center=(px, py))
            # simple soft shadow
            shadow_w = max(8, int(rect.w * 0.6))
            shadow_h = max(4, int(rect.h * 0.25))
            shadow = pygame.Surface((shadow_w, shadow_h), pygame.SRCALPHA)
            pygame.draw.ellipse(shadow, (0, 0, 0, 120), shadow.get_rect())
            screen.blit(shadow, (rect.centerx - shadow_w // 2, rect.bottom - shadow_h // 2))
            screen.blit(self.avatar, rect)
        else:
            # fallback: old circle
...
class AudioService:
    ...
    def __init__(self, model: WorldModel):
        ...
        # Variety memory (for contrast alternation)
        self._last_contrast = None

        # ---- Build the prompt context FIRST
        def _prompt_ctx():
            return dict(
                time_of_day=getattr(self.m, "time_of_day", None),
                weather=getattr(self.m, "weather", None),
                heat=getattr(self.m, "heat", 0.0),
                debt_pressure=getattr(self.m, "debt_pressure", 0.0),
                festival=getattr(self.m, "festival", False),
                cosmic_event=getattr(self.m, "cosmic_event", False),
                neighbor_contrast_prev=self._last_contrast,
            )

        # ---- Create the provider BEFORE starting the worker or queuing anything
        self.provider = LocalStableAudioProvider(context_fn=_prompt_ctx)

        # Listen for zone changes
        self.m.add_zone_changed_listener(self.on_zone_changed)

        # Preload theme or queue current tile
        if not self._maybe_preload(self.m.current_coord):
            self.request_generate(self.m.current_coord, priority=0, force=True)

        # Prepare neighbors (just queues – safe; provider already exists)
        self._reprioritize_all()
        self.prefetch_neighbors(self.m.current_coord, priority=1)

        # ---- Start the worker LAST, after provider exists and tasks may be queued
    ...
    def _worker_loop(self):
        ...
        while not self._worker_stop:
            ...
            try:
                ...
                z.generating = False
            z.loop = loop
            self._last_contrast = loop.provider_meta.get("contrast", self._last_contrast)
            self._prune_cache()
            ...
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    ...
                    if e.key in (pygame.K_ESCAPE, pygame.K_q):
                        ...
                        import importlib, promptgen
                        importlib.reload(promptgen)
                        print(f"[PromptGen] Reloaded {getattr(promptgen, 'SCHEMA_VERSION', '?')}")
                        # optional: regen current zone with new prompt
