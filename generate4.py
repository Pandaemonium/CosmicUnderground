...
PROMPT_HISTORY_FILE = "last_prompt.txt"
# 1. Try to load the last prompt from the history file
try:
    with open(PROMPT_HISTORY_FILE, "r") as f:
        default_prompt = f.read().strip()
except FileNotFoundError:
    # If the file doesn't exist, use a first-time default
    default_prompt = "Alien FM bass groove, no drums, mono low end, 120 BPM, D minor, 4-bar seamless loop"
...
# 3. Save the new prompt to the history file for the next run
...
for repeat_generation in range(number):
    ...
    sanitized_prompt = sanitized_prompt[:80]
