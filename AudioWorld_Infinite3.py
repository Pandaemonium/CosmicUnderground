...
# --- Display config ---
FULLSCREEN = True  # set default; can be toggled at runtime with F11 / Alt+Enter
...
class GameController:
    def __init__(self):
        ...
        def _apply_display_mode(fullscreen: bool):
            """
            Fullscreen with SCALED needs a non-zero *logical* size.
            We keep SCREEN_W/H as the logical size and let SDL scale to desktop.
            If SCALED+FULLSCREEN fails on a device, we fall back to raw FULLSCREEN.
            """
            flags = 0
            if fullscreen:
                try:
                    # SCALED + non-zero logical size (use your game’s logical size)
                    screen = pygame.display.set_mode((SCREEN_W, SCREEN_H), pygame.FULLSCREEN | pygame.SCALED)
                    # IMPORTANT: do NOT overwrite SCREEN_W/SCREEN_H here; they are the logical size.
                    return screen
                except pygame.error:
                    # Fallback: true fullscreen at desktop res, no SCALED
                    screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)
                    # In this path, we *are* rendering at real pixels; update the globals:
                    w, h = screen.get_size()
                    globals()['SCREEN_W'], globals()['SCREEN_H'] = w, h
                    return screen
            else:
                # Windowed + SCALED (keeps your logical size, scales nicely)
                screen = pygame.display.set_mode((SCREEN_W, SCREEN_H), pygame.SCALED)
                # Keep SCREEN_W/SCREEN_H unchanged (logical size).
                return screen

        
        self.screen = _apply_display_mode(FULLSCREEN)
        ...
        self.prompt_text = ""
        # Auto-hide prompt if you change zones
    ...
    def run(self):
        ...
        while running:
                ...
                if e.type == pygame.QUIT: running = False
                    ...
                    # Toggle prompt overlay
                        ...
                        if self.show_prompt:
                            ...
                            z = self.model.get_zone(self.model.current_coord)
                            if z.loop and z.loop.prompt:
                                self.prompt_text = z.loop.prompt
                            else:
                                # No generated loop yet; show a helpful note instead of guessing a prompt
                                self.prompt_text = "(No audio prompt yet — this zone’s loop hasn’t been generated.)"
                            self.show_prompt = True
                    elif e.key == pygame.K_F11 or (e.key == pygame.K_RETURN and (e.mod & pygame.KMOD_ALT)):
                        # toggle fullscreen on F11 or Alt+Enter
                        global FULLSCREEN
                        FULLSCREEN = not FULLSCREEN
                        # reapply mode and keep caption
                        self.screen = _apply_display_mode(FULLSCREEN)
                        pygame.display.set_caption(f"Alien DJ – Backend: {type(self.audio.provider).__name__}")
                    elif e.key == pygame.K_g:
                        c = self.model.current_coord
                        z = self.model.get_zone(c); z.loop = None
                        self.audio.request_generate(c, priority=0, force=True)  # top priority regen
            ...
            if dx or dy: self.model.move_player(dx, dy)
